...
class Encoder(object):
    ...
    def encode(self, bboxes_in, labels_in, criteria=0.5):
        ...
        bboxes_out[masks, :] = bboxes_in[best_dbox_idx[masks], :]	# 正样本的坐标是GT坐标,负样本坐标还是default box坐标,用不到
...
class DefaultBoxes(object):
    def __init__(self, fig_size, feat_size, steps, scales, aspect_ratios, scale_xy=0.1, scale_wh=0.2):
        ...
        #----------------------------------------#
        #   计算每层特征层的feature map size,每个层的框的大小
        #   300 / [8, 16, 32, 64, 100, 300]
        #----------------------------------------#
        fk = fig_size / np.array(steps)
        #----------------------------------------#
        #   每个预测特征层上预测的default box的 ratios宽高变化比例不包括1,因为默认就是
        #   [[2], [2, 3], [2, 3], [2, 3], [2], [2]]
        #----------------------------------------#
        ...
        for idx, sfeat in enumerate(self.feat_size):
            sk1 = scales[idx] / fig_size        # scale转为相对值[0-1]  scale/300 得到相对尺寸
            sk2 = scales[idx + 1] / fig_size    # scale转为相对值[0-1]  计算下一层的尺度,因为会用到
            sk3 = sqrt(sk1 * sk2)               # 两层相乘
            ...
            for alpha in aspect_ratios[idx]:
                w, h = sk1 * sqrt(alpha), sk1 / sqrt(alpha) # 分别乘除 2,0.5  3,0.33
                all_sizes.append((w, h))    # 2:1 等等
                all_sizes.append((h, w))    # 1:2 等等

            #----------------------------------------#
            #   计算当前特征层对应原图上的所有default box
            #----------------------------------------#
            for w, h in all_sizes:      # 遍历每一个default box
                for i, j in itertools.product(range(sfeat), repeat=2):  # i -> 行（y）， j -> 列（x）   # 对应的位置xy
                    #----------------------------------------#
                    #   计算每个default box的中心坐标（范围是在0-1之间）
                    #   + 0.5 是将位置移到框的中心(每个框的相对宽高都为1)
                    #----------------------------------------#
                    cx, cy = (j + 0.5) / fk[idx], (i + 0.5) / fk[idx]   # 相对坐标,相对于每个特征层的框的大小
                    self.default_boxes.append((cx, cy, w, h))           # 添加元组,变为二维列表

        #----------------------------------------#
        #   将default_boxes转为tensor格式  [num_anchors, 4] [8732, 4]
        #----------------------------------------#
...
class PostProcess(nn.Module):
    ...
    def scale_back_batch(self, bboxes_in, scores_in):
        ...
        #   超参数调整预测xywh
        #----------------------------------------#
        bboxes_in[:, :, :2] = self.scale_xy * bboxes_in[:, :, :2]   # 预测的x, y回归参数    0.1  计算损失时是10,这里要变回去,所以是0.1
        bboxes_in[:, :, 2:] = self.scale_wh * bboxes_in[:, :, 2:]   # 预测的w, h回归参数    0.2  计算损失时是5, 这里要变回去,所以是0.2
    ...
    def decode_single_new(self, bboxes_in, scores_in, criteria, num_output):
        ...
        """
        decode:
            input  : bboxes_in (Tensor 8732 x 4), scores_in (Tensor 8732 x num_classes) num_classes包括背景
            output : bboxes_out (Tensor nboxes x 4), labels_out (Tensor nboxes)
            criteria : IoU threshold of bboexes             IoU阈值
            max_output : maximum number of output bboxes    最大输出框的个数
        """
        device = bboxes_in.device
        num_classes = scores_in.shape[-1]

        #----------------------------------------#
        #   对越界的bbox进行裁剪,都是相对值,所以0~1之间
        #----------------------------------------#
        bboxes_in = bboxes_in.clamp(min=0, max=1)

        #----------------------------------------#
        #   复制num_classes次,为了和faster rcnn的处理方式一样,每个框都有类别
        #   [8732, 4] -> [8732, 21, 4]
        #----------------------------------------#
        bboxes_in = bboxes_in.repeat(1, num_classes).reshape(scores_in.shape[0], -1, 4)

        #----------------------------------------#
        #   对每个default box都创建label,长度是分类数
        #   create labels for each prediction
        #----------------------------------------#
        labels = torch.arange(num_classes, device=device)
        #----------------------------------------#
        ...
        #   remove empty boxes
        #----------------------------------------#
        ws, hs = bboxes_in[:, 2] - bboxes_in[:, 0], bboxes_in[:, 3] - bboxes_in[:, 1]   # 得到宽度和高度
        keep = (ws >= 1 / 300) & (hs >= 1 / 300)    # 宽高大于1个像素,是相对值,所以用 1/300
        ...
        #----------------------------------------#
        keep = keep[:num_output]
        bboxes_out = bboxes_in[keep, :] # 目标边界框
        scores_out = scores_in[keep]    # 类别概率
        labels_out = labels[keep]       # 类别
    ...
    def forward(self, bboxes_in, scores_in):
        ...
        #   bboxes: [batch, 8732, 4]
        #----------------------------------------#
        for bbox, prob in zip(bboxes.split(1, 0), probs.split(1, 0)):  # split参数 split_size, split_dim
            bbox = bbox.squeeze(0)  # bbox: [1, 8732, 4] -> [8732, 4]
            prob = prob.squeeze(0)  # prob: [1, 8732, num_classes] -> [8732, num_classes] num_classes包括背景
            #   目标边界框 类别概率 类别
