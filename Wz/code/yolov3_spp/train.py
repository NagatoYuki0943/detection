...
def train(hyp):
    ...
    #--------------------------------------------------------------------------------------#
    #   每训练64张图片才更新一次权重,batch小的话,比如4, 64/4=16,,没训练16张图片才更新一次权重
    #--------------------------------------------------------------------------------------#
    ...
    #   dataloader
    #-----------------------------------------------------------#
    nw = min([os.cpu_count(), batch_size if batch_size > 1 else 0, 8])  # number of workers 多线程载入数据
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if opt.notest is False or epoch == epochs - 1:
            ...
            coco_mAR = result_info[8]   # 平均召回率

            #-----------------------------------------------------------#
            #   通过tensorboard绘图写入
            #-----------------------------------------------------------#
            if tb_writer:
                #              giou               是否有物体         类别损失          总损失   学习率
            ...
            #-----------------------------------------------------------#
            # update best mAP(IoU=0.50:0.95)  最好的IoU
            #-----------------------------------------------------------#
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='cfg/my_yolov3.cfg', help="*.cfg path")      # 运行calculate_dataset.py 生成针对自己数据的yolov3文件
    parser.add_argument('--data', type=str, default='data/my_data.data', help='*.data path')    # 运行calculate_dataset.py 生成针对自己数据的data文件
    parser.add_argument('--hyp', type=str, default='cfg/hyp.yaml', help='hyperparameters path') # 超参数,不建议自己修改
    parser.add_argument('--multi-scale', type=bool, default=True,                               # 是否使用多尺度训练
                        help='adjust (67%% - 150%%) img_size every 10 batches')
    parser.add_argument('--img-size', type=int, default=512, help='test size')                  # 预测网络输入大小
    parser.add_argument('--rect', action='store_true', help='rectangular training')                 # 
    parser.add_argument('--savebest', type=bool, default=False, help='only save best checkpoint')   # 是否只保存最好的数据,每次验证后后会保存
    parser.add_argument('--notest', action='store_true', help='only test final epoch')                  # 训练才会才验证,会节约时间
    parser.add_argument('--cache-images', action='store_true', help='cache images for faster training') # 缓存图片,没什么用
    parser.add_argument('--weights', type=str, default='weights/yolov3-spp-ultralytics-512.pt',         # 预训练权重
                        help='initial weights path')
    parser.add_argument('--name', default='', help='renames results.txt to results_name.txt if supplied')   # 
    parser.add_argument('--device', default='cuda:0', help='device id (i.e. 0 or 0,1 or cpu)')              # 设备,第一块gpu
    parser.add_argument('--single-cls', action='store_true', help='train as single-class dataset')          # 单类别识别
    # 是否定界一部分权重 设置False后会冻结darknet53,其余全放开,设置为True就只会训练header部分
    parser.add_argument('--freeze-layers', type=bool, default=False, help='Freeze non-output layers')
    parser.add_argument("--amp", default=False, help="Use torch.cuda.amp for mixed precision training")     # 是否使用混合精度训练(需要GPU支持混合精度)
