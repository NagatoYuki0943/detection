...
def compute_loss(p, targets, model):
    ...
    device = p[0].device
    lcls = torch.zeros(1, device=device)  # 分类损失   Tensor(0)
    lbox = torch.zeros(1, device=device)  # 定位损失   Tensor(0)
    lobj = torch.zeros(1, device=device)  # object损失 Tensor(0)
    #------------------------------------------------#
    #   计算正样本
    #------------------------------------------------#
    ...
    #------------------------------------------------#
    #   2个二分类损失,针对目标和类别 BCEWithLogitsLoss = Sigmoid + BCELoss
    #   pos_weight: 平衡正负样本不均匀
    #------------------------------------------------#
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        nb = b.shape[0]  # number of positive samples 正样本个数
        if nb:
            #------------------------------------------------#
            #   对应匹配到正样本的预测信息
            #------------------------------------------------#
            ...
            #------------------------------------------------#
            pxy = ps[:, :2].sigmoid()                               # xy调整值偏移量
            pwh = ps[:, 2:4].exp().clamp(max=1E3) * anchors[i]      # wh调整值
