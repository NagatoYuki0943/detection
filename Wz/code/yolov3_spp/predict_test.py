...
def main():
    ...
    cfg = "cfg/yolov3-spp.cfg"  # 改成生成的.cfg文件,运行calculate_dataset.py生成
    ...
    with torch.inference_mode():
        ...
        #-----------------------------------------------------------#
        # 调整大小                                              auto 最大边长等比例为512,短边会用0进行填充
        #-----------------------------------------------------------#
        ...
        bboxes = pred[:, :4].detach().cpu().numpy() # 中心位置参数
        scores = pred[:, 4].detach().cpu().numpy()  # 是否有物体
        classes = pred[:, 5].detach().cpu().numpy().astype(np.int32) + 1  # 类别
