...
def anchor_fitness(k: np.ndarray, wh: np.ndarray, thr: float):  # mutation fitness
    r = wh[:, None] / k[None]   # 第一个维度后新增一个维度  box宽高 / anchor宽高 = 重合度   r: [bbox个数, 中心个数, 2]
    x = np.minimum(r, 1. / r).min(2)  # ratio metric    x: [bbox个数, 中心个数]
...
def main(img_size=512, n=9, thr=0.25, gen=1000):
    ...
    # s = wh.std(0)  # 标准差
    # k, dist = kmeans(wh / s, n, iter=30)  # points, mean distance 参数为 真实宽高/标准差， 中心个数， 迭代次数
    ...
    f, bpr = anchor_fitness(k, wh, thr) # 适应度和召回率
