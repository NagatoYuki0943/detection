...
def _resize_image(image, self_min_size, self_max_size):
    ...
    if max_size * scale_factor > self_max_size:
        scale_factor = self_max_size / max_size  # 将缩放比例设为指定最大边长和图片最大边长之比  调整后的小边长度 = 原小边长度 * (指定最大边长 / 原最小边长)
...
class GeneralizedRCNNTransform(nn.Module):
    ...
    #--------------------------------------------------------------------#
    #   找到输入batch图像的最大channel,height,width
    #--------------------------------------------------------------------#
    def max_by_axis(self, the_list):
        # type: (List[List[int]]) -> List[int]
        maxes = the_list[0] # 第一张图片
        for sublist in the_list[1:]:  # 后面的图片
    ...
    def batch_images(self, images, size_divisible=32):
        ...
        for img, pad_img in zip(images, batched_imgs):
            ...
            #   pad_img大小一致,img大小不一致,要将img填充到pad_img中
    ...
    def forward(self,
        ...
        images = self.batch_images(images)      # 将images打包成一个batch,图片大小一致

        #---------------------------------------#
        #   创建ImageList 图片宽高 Tensor
        #---------------------------------------#
