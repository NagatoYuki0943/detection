...
#---------------------------------------------------#
#   ROI输出部分,rroipooling + 展平和两个全连接层 + 分类预测和框的预测 + 后处理部分
#---------------------------------------------------#
class RoIHeads(torch.nn.Module):
    ...
    def __init__(self,
                 box_roi_pool,   # Multi-scale RoIAlign pooling     ROIAlign
                 box_head,       # TwoMLPHead                       展平和两个FC
                 box_predictor,  # FastRCNNPredictor                分类预测和框的预测
                 # Faster R-CNN training
                 fg_iou_thresh, bg_iou_thresh,  # default: 0.5, 0.5     正负样本临界值
                 batch_size_per_image, positive_fraction,  # default: 512(每张图片中选取多少个proposal计算fasterrcnn损失)  0.25(正样本比例)
                 bbox_reg_weights,  # None 超参数
                 # Faster R-CNN inference   移除低目标概率,fast rcnn中进行nms处理的阈值,对预测结果根据score排序取前100个目标
        ...
        self.box_roi_pool = box_roi_pool    # ROIAlign,调整为相同大小
        self.box_head = box_head            # 展平和两个FC
        self.box_predictor = box_predictor  # 分类预测和框的预测
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
            if gt_boxes_in_image.numel() == 0:  # 该张图像中没有gt框，为背景
                ...
                #----------------------------------------------------#
                #   计算proposal与每个gt_box匹配的iou最大值，并记录索引 这里这两个值都是0.5,所以小于的为-1,没有-2
                #   iou < low_threshold索引值为 -1， low_threshold <= iou < high_threshold索引值为 -2
                #----------------------------------------------------#
                matched_idxs_in_image = self.proposal_matcher(match_quality_matrix)

                #----------------------------------------------------#
                #   clamp限制最小值，防止匹配标签时出现越界的情况
                #   注意-1, -2对应的gt索引会调整到0,获取的标签类别为第0个gt的类别（实际上并不是）,后续会进一步处理
                #----------------------------------------------------#
                clamped_matched_idxs_in_image = matched_idxs_in_image.clamp(min=0)
                #----------------------------------------------------#
                #   获取proposal匹配到的gt对应标签  正样本
                #----------------------------------------------------#
    ...
    def subsample(self, labels):
        ...
        return sampled_inds # 返回正负样本inde
    ...
    def select_training_samples(self,
        ...
        #----------------------------------------------------#
        #   根据gt和proposal计算边框回归参数(真实框参数) 中心坐标,宽高
        #----------------------------------------------------#
    ...
    def forward(self,
        ...
        """
        Arguments:
            features (List[Tensor])             共享特征层
            proposals (List[Tensor[N, 4]])      rpn生成的预测框
            image_shapes (List[Tuple[H, W]])    预处理图像大小,大小相同,等比例缩放后填充
            targets (List[Dict])                真实目标分类
        """
        ...
        #----------------------------------------------------#
        # 将采集样本通过Multi-scale RoIAlign pooling层,调整为相同大小
