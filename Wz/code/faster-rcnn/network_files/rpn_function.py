...
class AnchorsGenerator(nn.Module):
    ...
    Arguments:
        sizes (Tuple[Tuple[int]]):           anchor大小
        aspect_ratios (Tuple[Tuple[float]]): anchor缩放比例
    """
    ...
    def __init__(self, sizes=(128, 256, 512), aspect_ratios=(0.5, 1.0, 2.0)):
        ...
        # sizes个数和aspect_ratios个数要相等
        assert len(sizes) == len(aspect_ratios)

        self.sizes = sizes
        self.aspect_ratios = aspect_ratios
        self.cell_anchors = None
        self._cache = {}    # 原图上生成的anchor全存到这里
    ...
    def generate_anchors(self, scales, aspect_ratios, dtype=torch.float32, device=torch.device("cpu")):
        ...
        h_ratios = torch.sqrt(aspect_ratios)    # ratios开根号得到每一个宽高的乘法因子
        w_ratios = 1.0 / h_ratios               # 宽高因子

        # [r1, r2, r3]' * [s1, s2, s3]   3个比例*5个框得到长度115的向量
        # number of elements is len(ratios)*len(scales)
        ws = (w_ratios[:, None] * scales[None, :]).view(-1) # 展平
        hs = (h_ratios[:, None] * scales[None, :]).view(-1)

        #---------------------------------------------------#
        #   left-top, right-bottom coordinate relative to anchor center(0, 0)
        #   生成的anchors模板都是以（0, 0）为中心的, shape [len(ratios)*len(scales), 4]
        #   中心为(0,0),左上角为(-ws, -hs), 右上角为(ws, hs),都除以2是因为ws和hs是宽高,不除2的话最后宽高就翻倍了
        #---------------------------------------------------#
    ...
    def grid_anchors(self, grid_sizes, strides):
        ...
        for size, stride, base_anchors in zip(grid_sizes, strides, cell_anchors):
            grid_height, grid_width = size          # 每一个预测特层的高宽
            stride_height, stride_width = stride    # 对应原图的步长
            ...
            #---------------------------------------------------#
            shifts_x = torch.arange(0, grid_width, dtype=torch.float32, device=device) * stride_width       # 相乘得到对应原图的坐标,是每个框的左上角点
            ...
            #   shape: [grid_width*grid_height, 4]  [点的个数, 4个偏移量] 左上角和右下角偏移量相同
            #---------------------------------------------------#
    ...
    #---------------------------------------------------#
    #   向前传播
    #   image_list: batch信息,包含图片和图像尺寸信息
    #   feature_maps: list 预测特征层信息,预测特政层的个数有几个它就有几个
    #---------------------------------------------------#
    def forward(self, image_list, feature_maps):
        ...
        #   计算特征层上的一步等于原始图像上的步长 = 原始宽高 / 特征层宽高
        #---------------------------------------------------#
        ...
        #   anchors是个list，每个元素为一张图像的所有anchors信息, anchors_per_image也是列表
        #---------------------------------------------------#
...
class RPNHead(nn.Module):
    ...
    def __init__(self, in_channels, num_anchors):   # num_anchors = 9
        super(RPNHead, self).__init__()
        # 3x3Conv 滑动窗口
        self.conv = nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1)
        # 1x1Conv 计算预测的目标分数（这里的目标只是指前景或者背景）    这里out为1, 用的二分类交叉熵损失,out=2使用交叉熵损失
        self.cls_logits = nn.Conv2d(in_channels, num_anchors, kernel_size=1, stride=1)
        # 1x1Conv 计算预测的目标bbox regression参数
...
def concat_box_prediction_layers(box_cls, box_regression):
    ...
    for box_cls_per_level, box_regression_per_level in zip(box_cls, box_regression):
        ...
        #   AxC anchor个数 = anchor * classes 这里只需要判断前景和背景,所以classes=1
        #---------------------------------------------------#
        ...
        # anchors_num_per_position  anchor个数,除以4得到
        A = Ax4 // 4
        # classes_num 分类数
        C = AxC // A

        #---------------------------------------------------#
        #   [N, anchor*1, h, w] -> [N, anchor*h*w, 1]
        #---------------------------------------------------#
        box_cls_per_level = permute_and_flatten(box_cls_per_level, N, A, C, H, W)
        box_cls_flattened.append(box_cls_per_level)

        #---------------------------------------------------#
        #   [N, anchor*4, h, w] -> [N, anchor*h*w, 4]
        #---------------------------------------------------#
        box_regression_per_level = permute_and_flatten(box_regression_per_level, N, A, 4, H, W)
        box_regression_flattened.append(box_regression_per_level)

    #---------------------------------------------------#
    #   拼接box,将多个预测特征层上的结果进行拼接,然后展平 [b, 1] / [b, 4]
    #---------------------------------------------------#
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def __init__(self, anchor_generator, head,
        ...
        self.head = head
        self.box_coder = det_utils.BoxCoder(weights=(1.0, 1.0, 1.0, 1.0))   # 将预测的bbox regression参数应用到anchors上得到最终预测bbox坐标
    ...
    #---------------------------------------------------#
    #   计算每个anchors最匹配的真实框gt，并将anchors进行分类，前景，背景以及废弃的anchors
    #   正样本mask=1,负样本=-1,中间样本=2
    #---------------------------------------------------#
    def assign_targets_to_anchors(self, anchors, targets):
        ...
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            if gt_boxes.numel() == 0:
                ...
                #---------------------------------------------------#
                #   计算anchors与真实bbox的iou信息,是一个矩阵
                #   代表每一个真实框和预测框的交并比, 0:真实框 1:预测框
                ...
                labels_per_image = labels_per_image.to(dtype=torch.float32) # 得到正样本模板

                #---------------------------------------------------#
                #   background (negative examples) 负样本
                #---------------------------------------------------#
                bg_indices = matched_idxs == self.proposal_matcher.BELOW_LOW_THRESHOLD  # -1 负样本模板
                labels_per_image[bg_indices] = 0.0

                #---------------------------------------------------#
                #   discard indices that are between thresholds 0.3~0.7之间的样本
                #---------------------------------------------------#
                inds_to_discard = matched_idxs == self.proposal_matcher.BETWEEN_THRESHOLDS  # -2 中间模板
                labels_per_image[inds_to_discard] = -1.0

            labels.append(labels_per_image) # 正样本mask=1,负样本=-1,中间样本=2
    ...
    def _get_top_n_idx(self, objectness, num_anchors_per_level):
        ...
        offset = 0
        # 遍历每个预测特征层上的预测目标概率信息
        for ob in objectness.split(num_anchors_per_level, 1):   # split(分隔的长度, 在哪一个维度上分隔)
            if torchvision._is_tracing():
                num_anchors, pre_nms_top_n = _onnx_get_num_anchors_and_pre_nms_top_n(ob, self.pre_nms_top_n())
            else:
                num_anchors = ob.shape[1]  # 预测特征层上的预测的anchors个数
                pre_nms_top_n = min(self.pre_nms_top_n(), num_anchors)  # 前n个或者所有anchor
            ...
            offset += num_anchors   # 偏移量,所有anchor都在一起,用完第一个,要加偏移,防止都在最前面取
    ...
    def filter_proposals(self, proposals, objectness, image_shapes, num_anchors_per_level):
        ...
        objectness = objectness.reshape(num_images, -1) # [b, -1]

        #---------------------------------------------------#
        #   Returns a tensor of size size filled with fill_value
        #   levels负责记录分隔不同预测特征层上的anchors索引信息
        #   torch.full(形状, 填充数据) idx是 0 1 2... 用来区别多层数据
        #---------------------------------------------------#
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        objectness = objectness.flatten()   # 二维边一维
        ...
        box_loss = det_utils.smooth_l1_loss(
            pred_bbox_deltas[sampled_pos_inds],     # 正样本预测回归参数
            regression_targets[sampled_pos_inds],   # 相对与正样本anchor的真实框回归参数
            beta=1 / 9,
            size_average=False,
        ) / (sampled_inds.numel())
        ...
        objectness_loss = F.binary_cross_entropy_with_logits(
            objectness[sampled_inds], labels[sampled_inds]  # 预测值和真实值
        )
    ...
    def forward(self,
        ...
        num_anchors_per_level = [s[0] * s[1] * s[2] for s in num_anchors_per_level_shape_tensors]   # c * h * w
        ...
        if self.training:
            ...
            #---------------------------------------------------#
            #   结合anchors以及对应的gt，计算regression参数 x, y, w, h
            #---------------------------------------------------#
            regression_targets = self.box_coder.encode(matched_gt_boxes, anchors)
            #---------------------------------------------------#
            #   objectness: 预测的前景概率
            #   pred_bbox_deltas: 预测坐标偏移量
            #   regression_targets
            #   labels: 正样本mask=1,负样本=-1,中间样本=2
            #   regression_targets: 真实框回归参数
            #   返回 计算目标预测概率损失,二分类交叉熵损失
            #---------------------------------------------------#
