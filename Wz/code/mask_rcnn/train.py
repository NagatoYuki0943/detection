...
def main(args):
    ...
    det_results_file = f"det_results{now}.txt"  # 保存目标检测
    seg_results_file = f"seg_results{now}.txt"  # 保存实例分割
    ...
    val_data_loader = torch.utils.data.DataLoader(val_dataset,
                                                  batch_size=1,     # batch_size为1,多了的话会填充0,影响最终结果
                                                  shuffle=False,
                                                  pin_memory=True,
                                                  num_workers=nw,
                                                  collate_fn=train_dataset.collate_fn)
    ...
    # define optimizer
    params = [p for p in model.parameters() if p.requires_grad] # 需要训练的权重
...
if __name__ == "__main__":
    ...
    parser.add_argument('--wd', '--weight-decay', default=1e-4, type=float,
                        ...
                        dest='weight_decay')
    # 针对torch.optim.lr_scheduler.MultiStepLR的参数    16, 22轮衰减学习率
    parser.add_argument('--lr-steps', default=[16, 22], nargs='+', type=int,
                        help='decrease lr every step-size epochs')
    # 针对torch.optim.lr_scheduler.MultiStepLR的参数    学习率衰减倍率
