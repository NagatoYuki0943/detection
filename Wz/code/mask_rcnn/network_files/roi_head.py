...
def maskrcnn_inference(x, labels):
    ...
    mask_prob = mask_prob[index, labels][:, None]   # [:, None] 新增一个维度 [n, 28, 28] => [n, 1, 28, 28]
...
def maskrcnn_loss(mask_logits, proposals, gt_masks, gt_labels, mask_matched_idxs):
    ...
    Args:
        mask_logits:    最终预测的mask 28x28
        proposals:      mask_roi之前的框
        gt_masks:       真实mask
        gt_labels:      全部真实label
        mask_matched_idxs: 真实框索引
    ...
    def forward(self,
        ...
        if self.has_mask():
            ...
            if self.training:
                ...
                num_images = len(proposals) # batch_size
                mask_proposals = []         # 存放计算损失的proposals
                pos_matched_idxs = []       # 存放对应真实框索引
                # 循环遍历每张图片
                for img_id in range(num_images):
                    pos = torch.where(labels[img_id] > 0)[0]            # 寻找对应gt类别大于0，即正样本
                    mask_proposals.append(proposals[img_id][pos])       # 找对应图片正样本的proposal
                    pos_matched_idxs.append(matched_idxs[img_id][pos])  # 找对应图片正样本的真实框索引
            else:
            ...
            #--------------------------------#
            # 采样图片到指定大小
            mask_features = self.mask_roi_pool(features, mask_proposals, image_shapes)  # mask_rcnn.py MultiScaleRoIAlign()
            mask_features = self.mask_head(mask_features)                               # mask_rcnn.py MaskRCNNHeads()
            mask_logits = self.mask_predictor(mask_features)                            # mask_rcnn.py MaskRCNNPredictor()  最终预测的mask 28x28
            ...
            if self.training:
                ...
                # 真实label
                gt_labels = [t["labels"] for t in targets]
                # 计算损失                     最终预测的mask mask_roi之前的框 真实mask 全部真实label 真实框索引
