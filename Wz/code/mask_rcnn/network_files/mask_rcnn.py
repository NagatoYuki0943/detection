...
class MaskRCNN(FasterRCNN):
    ...
    def __init__(
        ...
        if mask_roi_pool is None:
            # 输入特征转化为14x14大小                          在哪些特征上采样,backbone/resnet50_fpn_model.py 中有
        ...
        if mask_head is None:
            mask_layers = (256, 256, 256, 256)  # Mask分支中四个卷积的输出通道数
            mask_dilation = 1
            mask_head = MaskRCNNHeads(out_channels, mask_layers, mask_dilation)

        #-----------------------------------#
        # Mask分支预测 转置卷积+1x1Conv
        #-----------------------------------#
        if mask_predictor is None:
            mask_predictor_in_channels = 256
            mask_dim_reduced = 256              # 通过转置卷积后通道数为
        ...
        #--------------------------------#
        # MaskRCNN添加在roi_heads中 roi_head.py
        #--------------------------------#
...
class MaskRCNNPredictor(nn.Sequential):
    def __init__(self, in_channels, dim_reduced, num_classes):
        super().__init__(OrderedDict([
            ...
            # 1x1Conv out_channels = num_classes + 1
